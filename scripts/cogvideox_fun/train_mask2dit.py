...
from videox_fun.data.dataset_image_video import (ImageVideoControlDataset,
                                                PretrainVideoDataset, SFTVideoDataset,
                                                MultiShotVideoSampler,
                                                get_random_mask)
from videox_fun.models import (AutoencoderKLCogVideoX,
                              MultiPromptCogVideoXTransformer3DModel, T5EncoderModel,
                              T5Tokenizer)
...
def log_validation(vae, text_encoder, tokenizer, transformer3d, args, accelerator, weight_dtype, global_step):
    try:
        ...
        transformer3d_val = MultiPromptCogVideoXTransformer3DModel.from_pretrained(
            args.pretrained_model_name_or_path, args.attn_msk_version, subfolder="transformer",
        ).to(weight_dtype)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=2,
        help=(
            'The version number of the added attention mask in CogVideoX.'
        ),
    )
    parser.add_argument(
        "--cond_training",
        ...
        default=False,
        help=(
            'Whether to enable scene extension training during supervised finetuning.'
        ),
    )
    parser.add_argument(
        "--cond_training_prob",
        ...
        default=0.5,
        help=(
            'The probability of scene extension training during supervised finetuning.'
        ),
    )
...
def main():
    ...
    transformer3d = MultiPromptCogVideoXTransformer3DModel.from_pretrained(
        args.pretrained_model_name_or_path, args.attn_msk_version, subfolder="transformer"
    )
    ...
    if args.use_ema:
        ema_transformer3d = MultiPromptCogVideoXTransformer3DModel.from_pretrained(
            args.pretrained_model_name_or_path, args.attn_msk_version, subfolder="transformer"
        )
        ema_transformer3d = EMAModel(ema_transformer3d.parameters(), model_cls=MultiPromptCogVideoXTransformer3DModel, model_config=ema_transformer3d.config)
    ...
    if version.parse(accelerate.__version__) >= version.parse("0.16.0"):
        ...
        def load_model_hook(models, input_dir):
            if args.use_ema:
                ...
                _, ema_kwargs = MultiPromptCogVideoXTransformer3DModel.load_config(ema_path, return_unused_kwargs=True)
                load_model = MultiPromptCogVideoXTransformer3DModel.from_pretrained(
                    input_dir, subfolder="transformer_ema"
                )
                load_model = EMAModel(load_model.parameters(), model_cls=MultiPromptCogVideoXTransformer3DModel, model_config=load_model.config)
    ...
    if "pretrain" in args.train_data_meta:
        print(f"[RANK {accelerator.process_index}]Use PretrainVideoDataset!")
        train_dataset = PretrainVideoDataset(
        ...
        print(f"[RANK {accelerator.process_index}]Use SFTVideoDataset!")
        train_dataset = SFTVideoDataset(
            args.train_data_meta, args.train_data_dir,
            video_sample_size=args.video_sample_size, video_sample_stride=args.video_sample_stride, video_sample_n_frames=args.video_sample_n_frames, 
            video_repeat=args.video_repeat, 
            image_sample_size=args.image_sample_size,
    ...
    if args.enable_bucket:
        ...
        # DataLoaders creation:
        batch_sampler_generator = torch.Generator().manual_seed(args.seed)
        batch_sampler = MultiShotVideoSampler(RandomSampler(train_dataset, generator=batch_sampler_generator), train_dataset, args.train_batch_size)
        train_dataloader = torch.utils.data.DataLoader(
            train_dataset,
            batch_sampler=batch_sampler, 
            persistent_workers=True if args.dataloader_num_workers != 0 else False,
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if epoch == first_epoch and step == 0:
                ...
                for idx, (pixel_value, text) in enumerate(zip(pixel_values, texts)):
                    ...
                    text = texts[0][idx]
                    for i in range(1, args.num_scenes):
                        text += '\n' + texts[i][idx]
                ...
                if args.train_mode != "normal":
                    ...
                    for idx, (pixel_value, text) in enumerate(zip(mask_pixel_values, texts)):
                        pixel_value = pixel_value[None, ...]
                        text = texts[idx * args.num_scenes][0]
                        for i in range(1, args.num_scenes):
                            text += '\n' + texts[idx * args.num_scenes + i][0]
            ...
            with accelerator.accumulate(transformer3d):
                ...
                with torch.no_grad():
                    ...
                    def _batch_encode_vae(pixel_values):
                        ...
                        for i in range(0, pixel_values.shape[0], bs):
                            ...
                            pixel_values_bs_list = torch.split(pixel_values_bs, args.video_sample_n_frames, dim=2)
                            new_pixel_values_f = []
                            for j in range(args.num_scenes):
                                pixel_values_bs_f = vae.encode(pixel_values_bs_list[j])[0]
                                pixel_values_bs_f = pixel_values_bs_f.sample()
                                new_pixel_values_f.append(pixel_values_bs_f)
                            new_pixel_values.append(torch.cat(new_pixel_values_f, dim=2))
                ...
                if args.enable_text_encoder_in_dataloader:
                    ...
                    with torch.no_grad():
                        prompt_ids = tokenizer(
                            [text[idx] for idx in range(args.train_batch_size) for text in batch["text"]], 
                            max_length=args.tokenizer_max_length, 
                            padding="max_length", 
                            add_special_tokens=True, 
                            truncation=True, 
                        ...
                        prompt_embeds = prompt_embeds.reshape(args.train_batch_size, -1, prompt_embeds.shape[-1])
                ...
                if args.cond_training:
                    if random.random() > args.cond_training_prob:
                        cond_training = False
                        clip_idx = None
                        print(f"[RANK {accelerator.process_index}]Use normal training")
                    else:
                        cond_training = True
                        num_latents = latents.size(1) // args.num_scenes
                        clip_idx = args.num_scenes - 1
                        print(f"[RANK {accelerator.process_index}]Use conditional training, predict clip #{clip_idx}")
                else:
                    cond_training = False
                    clip_idx = None
                    print(f"[RANK {accelerator.process_index}]Use normal training")
                ...
                if cond_training:
                    ...
                    if clip_idx != 0:
                        noisy_latents[:, :clip_idx * num_latents] = latents[:, :clip_idx * num_latents]
                    if clip_idx != args.num_scenes - 1:
                        noisy_latents[:, (clip_idx + 1) * num_latents:] = latents[:, (clip_idx + 1) * num_latents:]
                else:
                    noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
                if noise_scheduler.config.prediction_type == "epsilon":
                    ...
                    if cond_training:
                        target = target[:, clip_idx * num_latents : (clip_idx + 1) * num_latents]
                else:
                ...
                noise_pred = transformer3d(
                    ...
                    timestep=timesteps,
                    timestep_conditioning=torch.zeros(1, dtype=timesteps.dtype, device=timesteps.device) if cond_training else None,
                    clip_idx=clip_idx,
                ...
                if cond_training:
                    loss = F.mse_loss(noise_pred[:, clip_idx * num_latents : (clip_idx + 1) * num_latents].float(), target.float(), reduction="mean")
                else:
