...
from videox_fun.models import (AutoencoderKLCogVideoX,
                              ...
                              T5Tokenizer)
from videox_fun.pipeline import (CondtionalMultiPromptCogVideoXFunPipeline,
...
def find_random_max_stride_sample(data_list, N, a, b):
    L = len(data_list)
    max_stride = None
...
def load_video_conds(video_cond_urls):
    video_conds = []
    for url in video_cond_urls[:2]:
        video_reader = VideoReader(url, ctx=decord.cpu(0))
        batch_index = find_random_max_stride_sample(list(range(len(video_reader))), video_length,
                                                        1, 3)
        pixel_values = video_reader.get_batch(batch_index)
        pixel_values = pixel_values.asnumpy().astype(np.uint8)
        pixel_values = torch.from_numpy(pixel_values).permute(0, 3, 1, 2).contiguous()
        pixel_values = pixel_values / 255.
        pixel_values = video_transforms(pixel_values).unsqueeze(0)
        video_conds.append(pixel_values.to(weight_dtype))
    return video_conds
...
prompt              = ["A white rat (Actor1) in a dark, sketchy cave/burrow blinks and shifts, looking right.", "A white rat (actor1) stands on its hind legs in a dark, cave-like setting, facing right. It remains stationary throughout the video.", "Three white rats with gray shading (actor1, actor2, actor3) stand in a dark, stone-like setting. Actor1 (left) gestures towards actor2 (center) while actor3 (right) observes, paw slightly raised."]
video_cond_urls     = ["asset/autoregressive_scene_expansion/scene_1.mp4", "asset/autoregressive_scene_expansion/scene_2.mp4"]
...
lora_weight         = 0.55
save_path           = "samples/mask2dit-cogvideox-5b-autoregressive-scene-expansion"
...
transformer = MultiPromptCogVideoXTransformer3DModel.from_pretrained(
    model_name, attn_msk_version,
...
if transformer.config.in_channels != vae.config.latent_channels:
    ...
    pipeline = CondtionalMultiPromptCogVideoXFunPipeline(
        vae=vae,
        tokenizer=tokenizer,
        text_encoder=text_encoder,
        transformer=transformer,
...
with torch.no_grad():
    ...
    video_conds = load_video_conds(video_cond_urls)
    sample = pipeline(
        prompt, 
